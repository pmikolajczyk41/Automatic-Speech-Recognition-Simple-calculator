...
def _filter_tokens(tokens: Iterable[Token], beam_size: int) -> List[Token]:
    ...
    return sorted(filtered, key=lambda t: t.log_probability, reverse=True)


def viterbi(initial_state: State, observations: Iterable[FeatVec], target_state: State,
            beam_size: int = sys.maxsize) -> Token:
    assert not initial_state.is_emitting
    active_tokens = [Token(initial_state)]
    for o in observations:
        active_tokens = _filter_tokens(_move_tokens(active_tokens, o), beam_size)
